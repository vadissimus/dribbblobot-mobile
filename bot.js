...
const DELAY_BETWEEN_MESSAGES = 5000; // 5 секунд задержки между сообщениями
...
async function scrapeDribbbleImages() {
  ...
  try {
    ...
    await page.goto('https://dribbble.com/shots/popular/mobile', {
    ...
    // Форматирование даты (ваш улучшенный вариант)
    ...
    for (const [index, image] of imagesData.entries()) {
      if (image.url) {
        const caption = `${currentDate} | Mobile | ${index + 1}\nШот: ${image.shotUrl}\nАвтор: ${image.additionalUrl}`;
...
scrapeDribbbleImages().catch(console.error);
